...
use crate::{prefs::Prefs, state::ManagerExt, thunderstore::Thunderstore};
...
fn get_packages(game: Game, prefs: &Prefs) -> Result<Option<Vec<PackageListing>>> {
    ...
    let result: Vec<PackageListing> =
        gale_util::fs::read_json(path).context("failed to deserialize cache")?;
...
pub fn write_packages(packages: &[&PackageListing], game: Game, prefs: &Prefs) -> Result<()> {
    ...
    gale_util::fs::write_json(cache_path(game, prefs), packages, JsonStyle::Compact)
